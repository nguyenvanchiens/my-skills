...
2. Route to the narrowest platform skill as soon as the stack is known:
   - Web: `aspnet-core`, `aspnet-mvc`, `minimal-apis`, `web-api`, `blazor`, `signalr`, `grpc`
...
3. Route cross-cutting work to the companion skill instead of keeping it inside generic `.NET` advice:
   ...
   - logging / observability: `aspnet-logging`
   - caching: `aspnet-caching`
   - health checks: `aspnet-health-checks`
   - background jobs / scheduled tasks: `background-jobs`
   - advanced auth (OAuth/OIDC/Identity/refresh tokens): `aspnet-auth-advanced`
   - testing runner (v2/v3, VSTest/MTP): `xunit`, `tunit`, `mstest`
   - testing design (mocks, integration, fixtures): `dotnet-testing-patterns`
...
- If the repo has `Views/` folder + Controllers returning `View()` (Razor Views), prefer `aspnet-mvc`.
